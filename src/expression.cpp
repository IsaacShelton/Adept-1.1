...
llvm::Value* LengthStringExp::assemble(Program& program, Function& func, AssemblyData& context, std::string* expr_type){
    ...
    length_string_values[1] = llvm::ConstantInt::get(context.context, llvm::APInt(64, this->value.length()));
...
UnsignedSizeExp::UnsignedSizeExp(ErrorHandler& err){
    flags = EXP_CONSTANT;
    errors = err;
}
UnsignedSizeExp::UnsignedSizeExp(uint64_t val, ErrorHandler& err){
    value = val;
    flags = EXP_CONSTANT;
    errors = err;
}
UnsignedSizeExp::UnsignedSizeExp(const UnsignedSizeExp& other) : PlainExp(other) {
    value = other.value;
    flags = EXP_CONSTANT;
}
UnsignedSizeExp::~UnsignedSizeExp(){}
llvm::Value* UnsignedSizeExp::assemble(Program& program, Function& func, AssemblyData& context, std::string* expr_type){
    if(expr_type != NULL) *expr_type = "usize";
    return llvm::ConstantInt::get(context.context, llvm::APInt(64, value, false));
}
std::string UnsignedSizeExp::toString(){
    return to_str(value) + "uz";
}
PlainExp* UnsignedSizeExp::clone(){
    return new UnsignedSizeExp(*this);
}

NumericIntegerExp::NumericIntegerExp(ErrorHandler& err){
    flags = EXP_CONSTANT;
    errors = err;
}
NumericIntegerExp::NumericIntegerExp(uint64_t val, ErrorHandler& err){
    value = val;
    flags = EXP_CONSTANT;
    errors = err;
}
NumericIntegerExp::NumericIntegerExp(const NumericIntegerExp& other) : PlainExp(other) {
    value = other.value;
    flags = EXP_CONSTANT;
}
NumericIntegerExp::~NumericIntegerExp(){}
llvm::Value* NumericIntegerExp::assemble(Program& program, Function& func, AssemblyData& context, std::string* expr_type){
    // TODO: Add type inference for numeric integer literals
    // TODO: Make TOKEN_NUMERIC_INT be generated by the lexer instead of a TOKEN_INT for integer literals
    if(expr_type != NULL) *expr_type = "int";
    return llvm::ConstantInt::get(context.context, llvm::APInt(32, (int64_t) value, true));
}
std::string NumericIntegerExp::toString(){
    return to_str(value);
}
PlainExp* NumericIntegerExp::clone(){
    return new NumericIntegerExp(*this);
}
...
llvm::Value* MemberExp::assemble_struct(Program& program, Function& func, AssemblyData& context, std::string* expr_type, Struct& target, llvm::Value* data, std::string& data_typename){
    ...
    indices[1] = llvm::ConstantInt::get(context.context, llvm::APInt(32, index, true));
...
llvm::Value* MemberExp::assemble_array(Program& program, Function& func, AssemblyData& context, std::string* expr_type, llvm::Value* data, std::string& data_typename){
    ...
    std::vector<llvm::Value*> indices(2);
    indices[0] = llvm::ConstantInt::get(context.context, llvm::APInt(32, 0, false));
    indices[1] = llvm::ConstantInt::get(context.context, llvm::APInt(32, index, false));
...
llvm::Value* SizeofExp::assemble(Program& program, Function& func, AssemblyData& context, std::string* expr_type){
    ...
    return llvm::ConstantInt::get(context.context, llvm::APInt(64, type_size, false));
}
...
llvm::Value* AllocExp::assemble_plain(Program& program, Function& func, AssemblyData& context, std::string* expr_type){
    ...
    malloc_args[0] = llvm::ConstantInt::get(context.context, llvm::APInt(64, type_size * amount, false));
...
llvm::Value* AllocExp::assemble_elements(Program& program, Function& func, AssemblyData& context, std::string* expr_type){
    ...
    malloc_args[0] = llvm::ConstantInt::get(context.context, llvm::APInt(64, type_size * amount * element_amount, false));
    llvm::Value* heap_memory = context.builder.CreateCall(malloc_function, malloc_args);

    std::vector<llvm::Value*> ctor_args = { heap_memory, llvm::ConstantInt::get(context.context, llvm::APInt(64, element_amount, false)) };
...
llvm::Value* DynamicAllocExp::assemble_plain(Program& program, Function& func, AssemblyData& context, std::string* expr_type){
    ...
    amount_llvm_value = context.builder.CreateBitCast(amount_llvm_value, llvm::Type::getInt64Ty(context.context));
    malloc_args[0] = context.builder.CreateMul(llvm::ConstantInt::get(context.context, llvm::APInt(64, type_size, false)), amount_llvm_value);
    llvm::Value* heap_memory = context.builder.CreateCall(malloc_function, malloc_args);
...
llvm::Value* DynamicAllocExp::assemble_elements(Program& program, Function& func, AssemblyData& context, std::string* expr_type){
    ...
    amount_llvm_value = context.builder.CreateBitCast(amount_llvm_value, llvm::Type::getInt64Ty(context.context));
    malloc_args[0] = context.builder.CreateMul(llvm::ConstantInt::get(context.context, llvm::APInt(64, type_size * element_amount, false)), amount_llvm_value);
    llvm::Value* heap_memory = context.builder.CreateCall(malloc_function, malloc_args);
