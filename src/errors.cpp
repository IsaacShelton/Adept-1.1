...
void ErrorHandler::panic_undeclared_method(const std::string& struct_name, const std::string& name, const std::vector<std::string>& args){
    std::string message = "Undefined method '" + struct_name + "." + name + "(";
    for(size_t i = 0; i != args.size(); i++){
        message += args[i];
        if(i + 1 != args.size()) message += ", ";
    }
